...
export default function CategoryDetailsPageClient() {
  ...
  useEffect(() => {
    if (!safeKey) return;
    const r = ref(db, `Categories/${safeKey}`);
    const unsub = onValue(
      r,
      (snap) => {
        const val = snap.val();
  ...
  useEffect(() => {
    if (!safeKey) return;
    const r = ref(db, `Categories/${safeKey}/Problems`);
    const unsub = onValue(r, (snap) => {
      const val = snap.val();
      setProblems(val ? Object.keys(val) : []);
    });
    return () => unsub();
  }, [safeKey]);
...
const goToProblem = (problemKey) => {
  router.push(`/ProblemDetailPages/${safeKey}/${problemKey}`);
};

  // Navigate to variant detail
  const goToVariant = (variantKey) => {
    router.push(`/variantsdetails?text=${encodeURIComponent(variantKey)}&topic=${encodeURIComponent(safeKey)}`);
  };
  ...
  const sections = data
    ? [
        ...
        data.Variants && { title: "Variants", content: <Section title="Variants" icon={BookOpen}><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">{Object.keys(data.Variants).map((item) => (
          <motion.button
            key={item}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => goToVariant(item)}
            className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-5 py-3 rounded-lg shadow-md hover:opacity-90 transition duration-200 flex items-center justify-between"
          >
            <span className="font-medium">{item.replace(/_/g, " ")}</span>
        ...
        problems.length > 0 && { title: "Problems", content: <Section title="Problems" icon={BookOpen}><div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">{problems.map((problem) => (
          <motion.button
            key={problem}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => goToProblem(problem)}
            className="bg-gradient-to-r from-green-600 to-teal-600 text-white px-5 py-3 rounded-lg shadow-md hover:opacity-90 transition duration-200 flex items-center justify-between"
          >
            <span className="font-medium">{problem.replace(/_/g, " ")}</span>
            <svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>
          </motion.button>
        ))}</div></Section> },
      ].filter(Boolean)
